...
export default function Settings({ settings, onUpdate }) {
  const [aiEndpoint, setAiEndpoint] = useState(settings.aiEndpoint || 'http://localhost:11434/api/generate')
  const [aiModel, setAiModel] = useState(settings.aiModel || 'llama3')
  const [apiKey, setApiKey] = useState(settings.apiKey || '')
  const [colorScheme, setColorScheme] = useState(settings.colorScheme || 'midnight')
  const [promptFileName, setPromptFileName] = useState(settings.promptFileName || '')
  const [promptContent, setPromptContent] = useState(settings.promptContent || '')
  const [showPrompt, setShowPrompt] = useState(false)
  const fileInputRef = useRef(null)

  const handleEndpointChange = (e) => {
    setAiEndpoint(e.target.value)
    onUpdate({ ...settings, aiEndpoint: e.target.value })
  }

  const handleModelChange = (e) => {
    setAiModel(e.target.value)
    onUpdate({ ...settings, aiModel: e.target.value })
  }
  ...
  const handleColorSchemeChange = (scheme) => {
    ...
    onUpdate({ ...settings, colorScheme: scheme })
  }
  ...
  const handlePromptUpload = (e) => {
    ...
    if (file.size > MAX_PROMPT_SIZE) {
      alert(`File too large. Maximum size is ${MAX_PROMPT_SIZE / 1024}KB`)
      return
    }
  ...
  return (
    <div className="settings-page">
      ...
      <div className="settings-section">
        ...
        <div className="setting-group">
          ...
          <input
            ...
            placeholder="Leave blank for local AI"
          />
        </div>
      </div>
      ...
      <div className="settings-section">
        ...
        <div className="color-schemes">
          {Object.entries(colorSchemes).map(([key, scheme]) => (
            <button
              key={key}
              className={`scheme-btn ${colorScheme === key ? 'active' : ''}`}
              onClick={() => handleColorSchemeChange(key)}
            >
              <span
                className="scheme-preview"
                style={{
                  background: `linear-gradient(135deg, ${scheme.bg} 50%, ${scheme.panelBg} 50%)`,
      ...
      <div className="settings-section">
        ...
        <p className="section-desc">Upload a .md file containing your video creation prompt template</p>
